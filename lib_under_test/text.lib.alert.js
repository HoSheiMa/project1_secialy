            ...
            function throw_alert(arr, qf, has_child_in_child, has_new_style_for_el2, has_new_style_for_el) {
                ...
                if (has_child_in_child == false) {
                    ...
                    el.appendChild(el2);
                    $(el2).addClass("hide").delay(10).queue(function(next) {
                        $(this).removeClass("hide");
                        next();
                    });
                    if (has_new_style_for_el) {
                        console.log(has_new_style_for_el.class)
                        if (typeof has_new_style_for_el.class == 'string') {
                            $(el).addClass('alert ' + has_new_style_for_el.class);
                        } else
                            $(el).addClass('alert').css(has_new_style_for_el.class);

                    } else {
                        has_new_style_for_el = {};
                        $(el).addClass('alert')
                    }

                    // animate to view 
                    if (has_new_style_for_el2) {
                        if (typeof has_new_style_for_el2.class == "string") {
                            $(el2).addClass(has_new_style_for_el2.class + ' hide');
                        } else {
                            $(el2).css(has_new_style_for_el2.class);
                            $(el2).addClass('hide');
                        }

                    } else {
                        $(el2).addClass('alert_Tab hide');
